...
const launchIcon  = createIcon("#10b981");
const burstIcon   = createIcon("#eab308");
const landingIcon = createIcon("#ef4444");
const balloonIcon = createIcon("#06b6d4", true);
...
function MapBoundsUpdater({ result }: { result: SimulationResult | null }) {
  ...
  useEffect(() => {
    if (result && result.trajectory.length > 0) {
      const bounds = L.latLngBounds(result.trajectory.map(pt => [pt.latitude, pt.longitude]));
...
function PlanBoundsUpdater({ cases, launchPos, targetPos }: {
  ...
  useEffect(() => {
    if (!cases || cases.length === 0) {
      if (targetPos) {
        map.fitBounds(L.latLngBounds([[launchPos.lat, launchPos.lng], [targetPos.lat, targetPos.lng]]), { padding: [60, 60] });
      }
      return;
    }
...
function MapClickHandler({ onMapClick, clickMode }: {
  ...
  const map = useMapEvents({
    click(e) {
      if (clickMode && onMapClick) onMapClick(e.latlng.lat, e.latlng.lng);
    },
  });
  useEffect(() => {
...
export function BalloonMap({
  ...
  const ascentPoints  = trajectory.filter(pt => pt.phase === "ascent").map(pt => [pt.latitude, pt.longitude] as [number, number]);
  const descentPoints = trajectory.filter(pt => pt.phase === "descent").map(pt => [pt.latitude, pt.longitude] as [number, number]);
  ...
  if (isAnimating && trajectory.length > 0) {
    ...
    activePath  = trajectory.slice(0, frame + 1).map(pt => [pt.latitude, pt.longitude] as [number, number]);
  ...
  return (
    <div className="h-full w-full rounded-lg overflow-hidden border border-border bg-muted/20 relative z-0">
      <MapContainer center={[launchPos.lat, launchPos.lng]} zoom={7} style={{ height: "100%", width: "100%" }} className="z-0">
        ...
        {isPlanMode && <PlanBoundsUpdater cases={planCases} launchPos={launchPos} targetPos={targetPos} />}
        ...
        {isPlanMode && planCases && planCases.map((fc, idx) => {
          ...
          return (
            <div key={idx}>
              <Polyline positions={traj.filter(p => p.phase === "ascent").map(p => [p.latitude, p.longitude] as [number, number])} color={color} weight={isSelected ? 3 : 2} opacity={opacity} dashArray="5,8" />
              <Polyline positions={traj.filter(p => p.phase === "descent").map(p => [p.latitude, p.longitude] as [number, number])} color={color} weight={isSelected ? 3 : 2} opacity={opacity} />
              <Marker position={[fc.simulation.landing.latitude, fc.simulation.landing.longitude]} icon={landIcon} opacity={opacity}>
        ...
        {simulationResult && !isAnimating && (
          <>
            ...
            {ascentPoints.length > 0 && <Marker position={ascentPoints[0]} icon={launchIcon}><Popup>Launch Site</Popup></Marker>}
            {burstPoint && <Marker position={[burstPoint.latitude, burstPoint.longitude]} icon={burstIcon}><Popup>Burst Point<br/>Alt: {(burstPoint.altitude / 1000).toFixed(1)} km</Popup></Marker>}
            {landingPoint && <Marker position={[landingPoint.latitude, landingPoint.longitude]} icon={landingIcon}><Popup>Landing Site</Popup></Marker>}
          </>
        )}
        ...
        {simulationResult && isAnimating && currentPos && (
          <>
            ...
            {burstPoint && animFrame >= trajectory.findIndex(pt => pt === burstPoint) && (
              <Marker position={[burstPoint.latitude, burstPoint.longitude]} icon={burstIcon}><Popup>Burst Point<br/>Alt: {(burstPoint.altitude / 1000).toFixed(1)} km</Popup></Marker>
            )}
            {landingPoint && animFrame >= trajectory.length - 1 && (
              <Marker position={[landingPoint.latitude, landingPoint.longitude]} icon={landingIcon}><Popup>Landing Site</Popup></Marker>
            )}
      ...
      <div className="absolute top-4 left-4 z-[400] pointer-events-none">
        <div className="px-3 py-1.5 bg-background/80 backdrop-blur border border-border rounded-md shadow-lg text-xs font-mono tracking-wider text-muted-foreground flex items-center gap-2">
          ...
          {clickMode === "launch" ? "출발지 클릭하세요" : clickMode === "target" ? "목적지 클릭하세요" : "SYSTEM ONLINE"}
        </div>
      </div>
    </div>
  );
